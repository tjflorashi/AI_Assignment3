...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        # define scope and all satisfying assignments list
        scope = [qi, qj]
        satTuples = []
        if i != j:
            for vali in qi.curDomain():
                for valj in qj.curDomain():
                    if (vali != valj) and (abs(vali - valj) != abs(i - j)):
                        satTuples.append([vali, valj])
        # initiate TableConstraint, then hasSupport and check are inherited from TableConstraint
        TableConstraint.__init__(self, name, scope, satTuples)

    # def check(self):
    #     return TableConstraint.check(self)
    #
    # def hasSupport(self, var, val):
    #     return TableConstraint.hasSupport(self, var, val)
...
class AllDiffConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        def valsNotEqual(l):
            ...
            # this boolean expression works since no duplication of value in set
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        totalNumOfRequiredValue = 0
        for var in self.scope():
            if var.isAssigned():
                if var.getValue() == self._required:
                    totalNumOfRequiredValue += 1
            else:
                return True
        return (self._lb <= totalNumOfRequiredValue) and (totalNumOfRequiredValue <= self._ub)
    ...
    def hasSupport(self, var, val):
        ...
        def isNumOfRequiredValueInRange(pairList):
            count = 0
            for (v, value) in pairList:
                if value == self._required:
                    count += 1
            return (self._lb <= count) and (count <= self._ub)

        def isPartialAssignmentValid(pairList):
            count = 0
            for (v, value) in pairList:
                if value == self._required:
                    count += 1
            if (count > self._ub) or (count + len(self.scope()) - len(pairList) < self._lb):
                return False
            return True

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        hasValidAssignment = findvals(varsToAssign, [(var, val)], isNumOfRequiredValueInRange, isPartialAssignmentValid)
