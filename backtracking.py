...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    if trace: print "==>Trying {}".format(nextVar.name())
    for value in nextVar.curDomain():
        nextVar.setValue(value)
        noDWO = True
        for constraint in csp.constraintsOf(nextVar):
            if constraint.numUnassigned() == 1:
                if FCCheck(constraint, nextVar, value) == "DWO":
                    noDWO = False
                    if trace: print "<==falsified constraint\n"
                    break
        if noDWO:
            newSolns = FC(unAssignedVars, csp, allSolutions, trace)
            if newSolns:
...
def GacEnforce(constraints, csp, reasonVar, reasonVal):
    ...
    while len(constraints) != 0:
        ...
        for var in constraint.scope():
            # since if var is already set a value, the curDomain method will return a domain list only contains the
            # value set, so do not need to worry about other possible values that could be set to var when
            # var = reasonVar
            for value in var.curDomain():
                if not constraint.hasSupport(var, value):
                    var.pruneValue(value, reasonVar, reasonVal)
                    if var.curDomainSize() == 0:
                        return "DWO"
                    for recheck in csp.constraintsOf(var):
                        if recheck != constraint and recheck not in constraints:
                            constraints.insert(0, recheck)
    return "OK"
